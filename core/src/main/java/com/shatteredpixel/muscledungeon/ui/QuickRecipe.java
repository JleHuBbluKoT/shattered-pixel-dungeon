...
import com.shatteredpixel.muscledungeon.items.Recipe;
...
import com.shatteredpixel.muscledungeon.items.food.MeatPie;
import com.shatteredpixel.muscledungeon.items.food.MysteryMeat;
import com.shatteredpixel.muscledungeon.items.food.Pasty;
import com.shatteredpixel.muscledungeon.items.food.StewedMeat;
...
import com.shatteredpixel.muscledungeon.items.potions.brews.BlizzardBrew;
import com.shatteredpixel.muscledungeon.items.potions.brews.CausticBrew;
import com.shatteredpixel.muscledungeon.items.potions.brews.InfernalBrew;
import com.shatteredpixel.muscledungeon.items.potions.brews.ShockingBrew;
import com.shatteredpixel.muscledungeon.items.potions.elixirs.ElixirOfAquaticRejuvenation;
import com.shatteredpixel.muscledungeon.items.potions.elixirs.ElixirOfDragonsBlood;
import com.shatteredpixel.muscledungeon.items.potions.elixirs.ElixirOfHoneyedHealing;
import com.shatteredpixel.muscledungeon.items.potions.elixirs.ElixirOfIcyTouch;
import com.shatteredpixel.muscledungeon.items.potions.elixirs.ElixirOfMight;
import com.shatteredpixel.muscledungeon.items.potions.elixirs.ElixirOfToxicEssence;
import com.shatteredpixel.muscledungeon.items.potions.exotic.ExoticPotion;
import com.shatteredpixel.muscledungeon.items.scrolls.Scroll;
import com.shatteredpixel.muscledungeon.items.scrolls.exotic.ExoticScroll;
import com.shatteredpixel.muscledungeon.items.spells.Alchemize;
import com.shatteredpixel.muscledungeon.items.spells.AquaBlast;
import com.shatteredpixel.muscledungeon.items.spells.ArcaneCatalyst;
import com.shatteredpixel.muscledungeon.items.spells.BeaconOfReturning;
import com.shatteredpixel.muscledungeon.items.spells.CurseInfusion;
import com.shatteredpixel.muscledungeon.items.spells.FeatherFall;
import com.shatteredpixel.muscledungeon.items.spells.MagicalInfusion;
import com.shatteredpixel.muscledungeon.items.spells.MagicalPorter;
import com.shatteredpixel.muscledungeon.items.spells.PhaseShift;
import com.shatteredpixel.muscledungeon.items.spells.ReclaimTrap;
import com.shatteredpixel.muscledungeon.items.spells.Recycle;
...
public class QuickRecipe extends Component {
	...
	public QuickRecipe(Recipe r, ArrayList<Item> inputs, final Item output) {
		...
		for (final Item in : inputs) {
			...
			curr = new ItemSlot(in) {
				...
				protected void onClick() {
					PixelDungeonsAndShadowDragons.scene().addToFront(new WndInfoItem(in));
				}
			};
		...
		if (hasInputs) {
			...
			if (!(PixelDungeonsAndShadowDragons.scene() instanceof AlchemyScene)) {
				arrow.enable(false);
			}
		} else {
		...
		this.output = new ItemSlot(output){
			@Override
			protected void onClick() {
				PixelDungeonsAndShadowDragons.scene().addToFront(new WndInfoItem(output));
			}
		};
		if (!hasInputs){
	...
	public class arrow extends IconButton {
		...
		protected void onClick() {
			...
			((AlchemyScene) PixelDungeonsAndShadowDragons.scene()).populate(ingredients, Dungeon.hero.belongings);
		}
