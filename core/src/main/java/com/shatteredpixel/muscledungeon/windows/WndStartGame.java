...
import com.shatteredpixel.muscledungeon.scenes.IntroScene;
...
public class WndStartGame extends Window {
	...
	public WndStartGame(final int slot){
		...
		if (Badges.isUnlocked(Badges.Badge.VICTORY)){
			IconButton challengeButton = new IconButton(
				...
				@Override
				protected void onClick() {
					PixelDungeonsAndShadowDragons.scene().add(new WndChallenges(SPDSettings.challenges(), true) {
	...
	private static class HeroBtn extends Button {
		...
		protected void onClick() {
			...
			if( !cl.isUnlocked() ){
				PixelDungeonsAndShadowDragons.scene().add(
	...
	private class HeroPane extends Component {
		...
		protected void createChildren() {
			...
			heroItem = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					PixelDungeonsAndShadowDragons.scene().add(new WndMessage(Messages.get(cl, cl.name() + "_desc_item")));
				}
			};
			...
			heroLoadout = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					PixelDungeonsAndShadowDragons.scene().add(new WndMessage(Messages.get(cl, cl.name() + "_desc_loadout")));
				}
			};
			...
			heroMisc = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					PixelDungeonsAndShadowDragons.scene().add(new WndMessage(Messages.get(cl, cl.name() + "_desc_misc")));
				}
			};
			...
			heroSubclass = new IconButton(new ItemSprite(ItemSpriteSheet.MASTERY, null)){
				...
				protected void onClick() {
					...
					PixelDungeonsAndShadowDragons.scene().add(new WndMessage(msg));
				}
			};
