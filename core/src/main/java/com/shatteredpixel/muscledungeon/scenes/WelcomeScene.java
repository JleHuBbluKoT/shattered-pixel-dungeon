...
import com.shatteredpixel.muscledungeon.Rankings;
...
import com.shatteredpixel.muscledungeon.effects.Fireball;
...
import com.shatteredpixel.muscledungeon.windows.WndStartGame;
...
public class WelcomeScene extends PixelScene {
	...
	private static int LATEST_UPDATE = PixelDungeonsAndShadowDragons.v0_7_2;
	...
	public void create() {
		...
		if (PixelDungeonsAndShadowDragons.versionCode == previousVersion) {
		...
		DarkRedButton okay = new DarkRedButton(Messages.get(this, "continue")){
			...
			protected void onClick() {
				...
				if (previousVersion == 0){
					SPDSettings.version(PixelDungeonsAndShadowDragons.versionCode);
					WelcomeScene.this.add(new WndStartGame(1));
				} else {
					updateVersion(previousVersion);
					PixelDungeonsAndShadowDragons.switchScene(TitleScene.class);
				}
			}
		};
		...
		if (previousVersion != 0){
			DarkRedButton changes = new DarkRedButton(Messages.get(this, "changelist")){
				...
				protected void onClick() {
					super.onClick();
					updateVersion(previousVersion);
					PixelDungeonsAndShadowDragons.switchScene(ChangesScene.class);
				}
			};
		...
		} else if (previousVersion <= PixelDungeonsAndShadowDragons.versionCode) {
	...
	private void updateVersion(int previousVersion){
		...
		if (previousVersion <= PixelDungeonsAndShadowDragons.v0_7_0c){
			Badges.loadGlobal();
		...
		if (previousVersion <= PixelDungeonsAndShadowDragons.v0_6_5c){
			Journal.loadGlobal();
		...
		if (previousVersion <= PixelDungeonsAndShadowDragons.v0_6_2e){
